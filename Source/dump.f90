...
IND_SELECT: SELECT CASE(IND)
   ...
   CASE(13)  ! MIXTURE FRACTION
      ...
      ENDDO
   CASE(14)  ! DIVERGENCE
      GAS_PHASE_OUTPUT_RES = (U(II,JJ,KK) - U(II-1,JJ,KK))*RDX(II) &
   ...
   CASE(89)
      ...
      IF (Z_INDEX > 0) THEN
         Y_SPECIES = ZZS(II,JJ,KK,Z_INDEX)
         RCON = SPECIES_MIXTURE(Z_INDEX)%RCON
      ELSEIF (Y_INDEX > 0) THEN
         ZZ_GET(1:N_TRACKED_SPECIES) = ZZS(II,JJ,KK,1:N_TRACKED_SPECIES)
         RCON = SPECIES(Y_INDEX)%RCON
         CALL GET_MASS_FRACTION(ZZ_GET,Y_INDEX,Y_SPECIES)
      ENDIF
      IF (DRY .AND. H2O_INDEX > 0) THEN
   ...
   CASE(135) ! SUM LUMPED MASS FRACTIONS STAR
      GAS_PHASE_OUTPUT_RES = SUM(ZZS(II,JJ,KK,1:N_TRACKED_SPECIES))

   CASE(136) ! SUM PRIMITIVE MASS FRACTIONS STAR
      ZZ_GET(1:N_TRACKED_SPECIES) = ZZS(II,JJ,KK,1:N_TRACKED_SPECIES)
      CALL GET_MASS_FRACTION_ALL(ZZ_GET,Y_ALL)
      GAS_PHASE_OUTPUT_RES = SUM(Y_ALL)
