...
SPECIES_LOOP: DO N=1,N_TOTAL_SCALARS
   ...
   FX_P(LBOUND(FX,1):,LBOUND(FX,2):,LBOUND(FX,3):) => FX(:,:,:,N)
   FY_P(LBOUND(FY,1):,LBOUND(FY,2):,LBOUND(FY,3):) => FY(:,:,:,N)
   FZ_P(LBOUND(FZ,1):,LBOUND(FZ,2):,LBOUND(FZ,3):) => FZ(:,:,:,N)
   ...
   !$OMP PARALLEL PRIVATE(IW,WC,BC,B1,II,JJ,KK,IIG,JJG,KKG,IOR,IC,U_TEMP,Z_TEMP,F_TEMP,U_WORK,Z_WORK,F_WORK)
   U_TEMP => U_WORK
   F_TEMP => F_WORK
   Z_TEMP => Z_WORK
   !$OMP DO
...
FACE_CORRECTION_IF: IF (FLUX_LIMITER_MW_CORRECTION) THEN
   ...
   FX_P(LBOUND(FX,1):,LBOUND(FX,2):,LBOUND(FX,3):) => FX(:,:,:,0)
   FY_P(LBOUND(FY,1):,LBOUND(FY,2):,LBOUND(FY,3):) => FY(:,:,:,0)
   FZ_P(LBOUND(FZ,1):,LBOUND(FZ,2):,LBOUND(FZ,3):) => FZ(:,:,:,0)
   ...
   !$OMP PARALLEL PRIVATE(IW,WC,BC,B1,II,JJ,KK,IIG,JJG,KKG,IOR,IC,U_TEMP,Z_TEMP,F_TEMP,U_WORK,Z_WORK,F_WORK,ZZ_GET,MW_F)
   U_TEMP => U_WORK
   F_TEMP => F_WORK
   Z_TEMP => Z_WORK
   !$OMP DO
